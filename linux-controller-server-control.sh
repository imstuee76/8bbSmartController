...
manual_start() {
  ...
  local server_script="$APP_ROOT/linux-controller-server.sh"
  ...
  (
    ...
    if [[ -x "$server_script" ]]; then
      nohup "$server_script" >>"$activity" 2>>"$errors" &
    else
      log "Server script missing at $server_script. Falling back to direct uvicorn start."
      export SMART_CONTROLLER_DATA_DIR="$DATA_DIR"
      export SMART_CONTROLLER_APP_ROOT="$APP_ROOT"
      export CONTROLLER_WEB_DIR="$web_dir"
      nohup python3 -m uvicorn app.main:app --app-dir "$APP_ROOT/flasher-web" --host "$host" --port "$port" >>"$activity" 2>>"$errors" &
    fi
    echo "$!" >"$PID_FILE"
  )
  local pid
